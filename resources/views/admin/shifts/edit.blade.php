...
<div style="background:#f0f4f0;min-height:100vh;padding:2rem 1rem;">
  <div class="container" style="max-width:640px;">
    ...
    <div class="mb-4">
      <h1 style="font-size:22px;font-weight:500;color:#085041;margin-bottom:4px;">Edit shift</h1>
      <p style="font-size:13px;color:#888780;margin:0;">Update shift assignment</p>
    </div>
      ...
      <form method="POST" action="{{ route('admin.shifts.update', $shift) }}">
          ...
          <select id="route_id" name="route_id"
            ...
            @foreach($routes as $route)
              <option value="{{ $route->id }}"
                      {{ old('route_id', $shift->route_id) == $route->id ? 'selected' : '' }}>
                {{ $route->name }}
              </option>
            @endforeach
          </select>
          ...
          <select id="driver_id" name="driver_id"
            ...
            @foreach($drivers as $driver)
              <option value="{{ $driver->id }}"
                      {{ old('driver_id', $shift->driver_id) == $driver->id ? 'selected' : '' }}>
                {{ $driver->name }}
              </option>
            @endforeach
          </select>
        ...
        <div class="mb-3">
          ...
          <select id="bus_id" name="bus_id"
            ...
            @foreach($buses as $bus)
              <option value="{{ $bus->id }}"
                      {{ old('bus_id', $shift->bus_id) == $bus->id ? 'selected' : '' }}>
        ...
        <div class="mb-4">
          ...
          <select id="status" name="status"
            ...
            <option value="scheduled" {{ old('status', $shift->status) === 'scheduled' ? 'selected' : '' }}>Scheduled</option>
            <option value="in_progress" {{ old('status', $shift->status) === 'in_progress' ? 'selected' : '' }}>In progress</option>
            <option value="completed" {{ old('status', $shift->status) === 'completed' ? 'selected' : '' }}>Completed</option>
            <option value="cancelled" {{ old('status', $shift->status) === 'cancelled' ? 'selected' : '' }}>Cancelled</option>
          </select>
