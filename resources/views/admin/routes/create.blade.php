      ...
      <form method="POST" action="{{ route('admin.routes.store') }}">
        ...
        <div class="mb-3">
          ...
          <textarea id="description"
                    ...
                    placeholder="Brief description of the route">{{ old('description') }}</textarea>
          @error('description')
            <div class="invalid-feedback">{{ $message }}</div>
          @enderror
        </div>
          ...
          <select id="start_stop_id"
            ...
            @foreach($stops as $stop)
              <option value="{{ $stop->id }}"
                      {{ old('start_stop_id') == $stop->id ? 'selected' : '' }}>
                {{ $stop->name }}
              </option>
            @endforeach
          </select>
        ...
        <div class="mb-3">
          ...
          <select id="end_stop_id"
            ...
            @foreach($stops as $stop)
              <option value="{{ $stop->id }}"
                      {{ old('end_stop_id') == $stop->id ? 'selected' : '' }}>
                {{ $stop->name }}
              </option>
            @endforeach
          </select>
        ...
        <div class="mb-3">
          ...
          <select id="return_route_id"
            ...
            @foreach($routes as $r)
              <option value="{{ $r->id }}"
                      {{ old('return_route_id') == $r->id ? 'selected' : '' }}>
                {{ $r->name }}
              </option>
            @endforeach
          </select>
